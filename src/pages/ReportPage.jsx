...
import {
  ...
  getBrandData, getBrandFiles, fetchAndParseExcel, parseAdData
} from '../utils/googleApi';
import { generateRecommendations } from '../utils/claudeApi';
import { writeRecommendations } from '../utils/googleApi';
...
export default function ReportPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        gapiRef.current = await initGoogleApi();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const match = files.find(f => f.name.includes(selectedWeek.replace('/', '-').slice(0,10)));
        if (match) {
          const rows = await fetchAndParseExcel(match.id);
          setAdData(parseAdData(rows));
        }
      } catch (e) {
        console.error('Excel 讀取失敗', e);
      }
    })();
  ...
  const handleGenerateSuggestions = async () => {
    ...
    try {
      const suggestions = await generateRecommendations({
        ...
        adData,
        socialData: {
          fb: parseInt(week['FB追蹤'] || 0),
          ig: parseInt(week['IG追蹤'] || 0),
          line: parseInt(week['LINE好友'] || 0),
          fbGrowth: calcGrowth('FB追蹤'),
          igGrowth: calcGrowth('IG追蹤'),
          lineGrowth: calcGrowth('LINE好友')
        }
      });
  ...
  if (loading) return (
    <div className="loading-screen">
      ...
      <p className="loading-sub">資料載入中...</p>
    </div>
  );
      ...
      <header className="report-header">
        ...
        <div className="week-selector">
          {allData.map(d => (
            <button
              key={d['週期']}
              className={`week-btn ${d['週期'] === selectedWeek ? 'active' : ''}`}
          ...
          <div className="metric-grid">
            {[
              ...
              <div key={m.label} className="metric-card">
                ...
                <p className={`metric-growth ${m.growth >= 0 ? 'pos' : 'neg'}`}>
                  {m.growth >= 0 ? '+' : ''}{m.growth} 人
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="table-wrap">
            <table className="ad-table">
              <thead>
                <tr>
                  <th>廣告類型</th>
                  <th className="num">花費(元)</th>
                  <th className="num">成果</th>
                  <th className="num">成本/次</th>
                  <th className="num">觸及</th>
                  <th>評級</th>
                </tr>
              </thead>
              <tbody>
                {adData.map(a => (
                  <tr key={a.type}>
                    <td>{a.type}</td>
                    <td className="num">${a.spend.toLocaleString()}</td>
                    <td className="num">{a.result.toLocaleString()}</td>
                    <td className="num">${a.cost}</td>
                    <td className="num">{a.reach.toLocaleString()}</td>
                    <td><GradeBadge grade={a.grade} label={a.label} /></td>
                  </tr>
                ))}
                <tr className="total-row">
                  ...
                  <td className="num">—</td>
                  <td className="num">—</td>
                  <td className="num">{totalReach.toLocaleString()}</td>
                  <td></td>
                </tr>
              </tbody>
            </table>
        ...
        {allData.length > 1 && (
          <section className="report-section">
            ...
            <div className="chart-wrap">
              <Line data={histChartData} options={histOptions} />
            </div>
